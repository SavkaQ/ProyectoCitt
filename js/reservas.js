...
(function(exports) {
  ...
  function manejarEnvioReserva(evento) {
    ...
    const { usuario, inventario, reservas } = obtenerDatosBase();
    ...
    const registro = {
      ...
      tipo: datos.get('tipo'),
      referenciaId: datos.get('referenciaId'),
    ...
    if (!registro.nombre || !registro.rut) {
    ...
    if (registro.tipo === 'articulo') {
      const articulo = inventario.find((item) => item.id === registro.referenciaId);
      if (!articulo) {
      ...
      const espacios = exports.storage.obtenerDatos('espacios');
      const espacio = espacios.find((item) => item.id === registro.referenciaId);
      if (!espacio) {
  ...
  function manejarAccionReserva(evento) {
    ...
    reservas[indice].estado = accion === 'aprobar' ? 'aprobada' : 'rechazada';
    exports.storage.guardarDatos('reservas', reservas);
    renderizarReservasPendientes();
    renderizarMisReservas();
    renderizarResumenReservas();
